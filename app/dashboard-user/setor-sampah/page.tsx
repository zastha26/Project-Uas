...
const jenisSampah = [
  { id: "plastik-pet", nama: "Plastik PET (Botol)", harga: 1500, satuan: "kg", kategori: "Plastik" },
  { id: "plastik-hdpe", nama: "Plastik HDPE", harga: 1200, satuan: "kg", kategori: "Plastik" },
  { id: "plastik-pp", nama: "Plastik PP", harga: 800, satuan: "kg", kategori: "Plastik" },
  { id: "plastik-campur", nama: "Plastik Campur", harga: 500, satuan: "kg", kategori: "Plastik" },
  { id: "kertas-koran", nama: "Kertas Koran", harga: 800, satuan: "kg", kategori: "Kertas" },
  { id: "kertas-hvs", nama: "Kertas HVS", harga: 1000, satuan: "kg", kategori: "Kertas" },
  { id: "kardus", nama: "Kardus", harga: 700, satuan: "kg", kategori: "Kertas" },
  { id: "kertas-campur", nama: "Kertas Campur", harga: 400, satuan: "kg", kategori: "Kertas" },
  { id: "kaleng-aluminium", nama: "Kaleng Aluminium", harga: 10000, satuan: "kg", kategori: "Logam" },
  { id: "kaleng-seng", nama: "Kaleng Seng", harga: 1800, satuan: "kg", kategori: "Logam" },
  { id: "besi", nama: "Besi", harga: 2500, satuan: "kg", kategori: "Logam" },
  { id: "tembaga", nama: "Tembaga", harga: 45000, satuan: "kg", kategori: "Logam" },
  { id: "botol-kaca-bening", nama: "Botol Kaca Bening", harga: 500, satuan: "kg", kategori: "Kaca" },
  { id: "botol-kaca-warna", nama: "Botol Kaca Warna", harga: 400, satuan: "kg", kategori: "Kaca" },
];
...
export default function SetorSampahPage() {
  ...
  const filteredJenis = filterKategori === "Semua" ? jenisSampah : jenisSampah.filter(j => j.kategori === filterKategori);
  ...
  const handleSubmit = () => {
    if (setoran.length === 0 || !userEmail) return;

    // Simpan transaksi
    const transactions = JSON.parse(localStorage.getItem("banksampah_transactions") || "[]");
    ...
    localStorage.setItem("banksampah_transactions", JSON.stringify(transactions));
    ...
    // Update di list users juga
    const users = JSON.parse(localStorage.getItem("banksampah_users") || "[]");
    const updated = users.map((u: { email: string }) => u.email === userEmail ? { ...u, saldo: user.saldo } : u);
      ...
      {showSuccess && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-3xl p-8 max-w-sm mx-4 text-center shadow-2xl animate-scale-in">
            ...
            <p className="text-gray-500 mb-4">Setoran Anda telah tercatat dan saldo telah ditambahkan.</p>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
              {kategoriList.map((kategori) => (
                <button key={kategori} onClick={() => setFilterKategori(kategori)}
                  className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all ${filterKategori === kategori ? "bg-green-500 text-white shadow-sm shadow-green-500/25" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
                  {kategori}
                </button>
              ))}
            </div>
              ...
              <div className="sm:col-span-2">
                ...
                <select value={selectedJenis} onChange={(e) => setSelectedJenis(e.target.value)}
                  ...
                  {filteredJenis.map((jenis) => (
                    <option key={jenis.id} value={jenis.id}>{jenis.nama} - Rp {jenis.harga.toLocaleString("id-ID")}/{jenis.satuan}</option>
                  ))}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Berat (kg)</label>
                <input type="number" value={berat} onChange={(e) => setBerat(e.target.value)} placeholder="0" min="0.1" step="0.1"
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all" />
              </div>
            </div>

            <button onClick={tambahSetoran} disabled={!selectedJenis || !berat || Number(berat) <= 0}
                    ...
                    <div key={index} className="p-4 flex items-center justify-between bg-white hover:bg-gray-50/50 transition-colors animate-fade-in-up">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-500">{item.berat} kg x Rp {item.harga.toLocaleString("id-ID")}</div>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="font-bold text-green-600">Rp {item.subtotal.toLocaleString("id-ID")}</div>
                        <button onClick={() => hapusSetoran(index)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all">
        ...
        <div className="lg:col-span-1 animate-fade-in-up">
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 sticky top-24">
            ...
            <button onClick={handleSubmit} disabled={setoran.length === 0}
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm text-green-700">
                  ...
                  <ul className="space-y-1.5 text-green-600">
                    <li className="flex items-start gap-1.5"><span className="mt-1.5 w-1 h-1 bg-green-400 rounded-full flex-shrink-0"></span>Bersihkan sampah sebelum disetor</li>
                    <li className="flex items-start gap-1.5"><span className="mt-1.5 w-1 h-1 bg-green-400 rounded-full flex-shrink-0"></span>Pisahkan berdasarkan jenis</li>
                    <li className="flex items-start gap-1.5"><span className="mt-1.5 w-1 h-1 bg-green-400 rounded-full flex-shrink-0"></span>Harga dapat berubah sewaktu-waktu</li>
                  </ul>
                </div>
              </div>
            </div>
